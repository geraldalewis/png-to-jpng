...
var supported = isFN(typeof HTMLCanvasElement) && 
                ...
                isFN(typeof FileReader) && !!(FileReader.prototype.readAsDataURL);
...
function free(obj){
  const canvas = (obj instanceof HTMLCanvasElement) ? obj : obj.canvas;
...
function createJPNGCanvas(png){
  var w = png.width, h = png.height,
      rgb = createRGB(png),
      a = createAlpha(png),
      height = h<<1,
      ctx = context2d(w, height);
...
function ensureQuality(quality){
...
function createJPNGBlob(png, quality, callback){
  const canvas = createJPNGCanvas(png);
  toBlob(canvas, (blob) => { callback(blob); }, 'image/jpeg', ensureQuality(quality));
}

function createJPNGDataURL(png, quality){
  const canvas = createJPNGCanvas(png);
  const url = canvas.toDataURL('image/jpeg', ensureQuality(quality));
  free(canvas);
  return url;
}

function createCompositedJpngDataURL(canvas, w, h, quality, callback){
  const dataURL = canvas.toDataURL('image/jpeg', ensureQuality(quality));
  jpng.toDataURL(dataURL, w, h, callback);
};
  ...
  img.onload = () => {
    ...
    if (hasTransparency(img)) {
      canCompress(img, size, 
        ...
        () => { reject({ img, src, size, error: errors.cannotCompress }) }
      );
    }
    else reject({ img, src, size, error: errors.notTransparent });
  };
...
function findQualityForRatio(png, size, ratio, lower, upper, callback){
  const tolerance = 1/64, qTolerance = 1/128;
...
function pruneVersions(results, size){
  ...
  return results.filter( (version, index, array) => {
    ...
    version.percent = (percent >= 0 ? '+' : '') + percent;
...
const toJPNG = {
  supported: supported,
  readFile: readFile,
  createJPNGBlob: createJPNGBlob,
  createCompositedJpngDataURL: createCompositedJpngDataURL,
  createJPNGDataURL: createJPNGDataURL,
  createVersions: createVersions,
  
  createJPNGCanvas: createJPNGCanvas
};

export default toJPNG;